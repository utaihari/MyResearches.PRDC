...
int PRDC_SAMEDICS_TEST(string dataset_path, int method_flag, int LOOP,
	...
	for (auto& c : classified_table) {
		c = vector<vector<int>>(classes.size());
		for (auto&d : c) {
			d = vector<int>(classes.size(), 0);
		}
	}
...
#pragma omp parallel for
		for (int i = 0; i < (int) test_contents.size(); ++i) {
			...
			classified_table[p][(int) test_class.at(i)][(int) r]++;
...
#pragma omp parallel for
	...
	for (int i = 0; i < (int) flags.size(); ++i) {
		...
		sorted_table = MakeRanking(classified_table[i], classes);
		for (int i = 0; i < 10; ++i) {
			cout << sorted_table.at(i).second << " :"
